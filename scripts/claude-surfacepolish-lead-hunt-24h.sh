...
AGENT_FILE="$DATA_DIR/surfacepolish-agent.md"
ARCHIVE_FILE="$DATA_DIR/surfacepolish-customer-master.md"
LOG_FILE="$APP_DIR/logs/claude-surfacepolish-lead-hunt-24h.log"
PID_FILE="$APP_DIR/logs/claude-surfacepolish-lead-hunt-24h.pid"
PAUSE_FILE="$APP_DIR/logs/lead-hunt-surfacepolish.paused"
...
while [ "$(date +%s)" -lt "$END_AT" ]; do
  ...
  ITER=$((ITER+1))
  TS=$(date '+%Y-%m-%d %H:%M:%S')
  BACKUP="$DATA_DIR/backups/surfacepolish-customer-master-$(date '+%Y%m%d-%H%M%S')-iter${ITER}.md"
