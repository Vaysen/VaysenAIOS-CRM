...
describe('external email brand policy', () => {
  const safeBody = [
    ...
    '<p><a href="https://vaysen.com">Vaysen Packaging</a></p>',
    '<p>To unsubscribe, reply with Unsubscribe.</p>',
  ].join('');

  it('uses Vaysen Packaging when company branding is absent or retired', () => {
  ...
  it('adds the Vaysen website when a body has no configured website', () => {
    const html = ensureCompanyWebsite('<p>Custom packaging quotation and sample support.</p>');
    expect(html).toContain('href="https://vaysen.com"');
  ...
  it('accepts a complete Vaysen deliverable', () => {
