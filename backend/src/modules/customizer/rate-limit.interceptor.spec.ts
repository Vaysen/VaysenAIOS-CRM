...
describe('customizer rate-limit client identity', () => {
  it('ignores a forged X-Forwarded-For prefix when Nginx supplies X-Real-IP', () => {
    const base = { headers: { 'x-real-ip': '10.10.5.9' }, socket: { remoteAddress: '10.20.0.2' } };
    expect(extractRateLimitClientIp({ ...base, headers: { ...base.headers, 'x-forwarded-for': '1.1.1.1, 10.10.5.9' } }))
      .toBe('10.10.5.9');
    expect(extractRateLimitClientIp({ ...base, headers: { ...base.headers, 'x-forwarded-for': '8.8.8.8, 10.10.5.9' } }))
      .toBe('10.10.5.9');
  });
