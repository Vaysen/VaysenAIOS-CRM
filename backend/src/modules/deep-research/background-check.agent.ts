...
@Injectable()
export class BackgroundCheckAgent {
  constructor(
  ...
  async research(
    ...
    if (!gathered?.html || !gathered?.json) {
      throw new Error(gathered?.error || '深度背调采集失败，未获得可核验的公开证据。');
    }

    const title = `${lead.companyName} - AI深度背调报告`;
    const reportId = await this.archiveReport(
      lead.id, lead.companyId, 'full', title, gathered.html, gathered.json, userId, options.agentRunId,
    );
