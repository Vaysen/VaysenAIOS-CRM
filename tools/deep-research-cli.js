#!/usr/bin/env node
/**
 * Vaysen AI CRM Deep Research CLI
 *
 * Standalone research tool — uses real web search + Zhipu GLM API.
 * Designed to be called from the Vaysen AI CRM backend via child_process.
 *
 * Usage:
...
function buildSystemPrompt(type) {
  const prompts = {
    full: `你是一位资深的国际B2B包装行业客户背调分析师。示例贸易公司（Example Trading Company）主营快递袋、牛皮纸袋、垃圾袋、自封袋及其他可定制软包装。请根据提供的真实网络数据，用中文撰写一份专业的客户背景调查报告，并从重复采购、定制需求、材质、尺寸、印刷、MOQ与交付条件评估合作匹配度。
...
${bodyHtml}
<div class="footer"><p>本报告基于公开可查的网络信息编制，仅供参考。</p><p>Generated by Vaysen AI CRM — AI Deep Research Engine</p></div>
</div></body></html>`;
}
