...
const CRM_CHANNEL = 'jorigin-crm';
const HTTP_INGRESS_CHANNEL = 'webchat';
const CRM_HTTP_SESSION_PATTERN = /^agent:jorigin-crm:(jorigin-crm:[a-f0-9]{64})$/;
...
export function resolveTrustedReadOnlyInvokeActor(toolContext = {}) {
  const channel = typeof toolContext.messageChannel === 'string'
    ? toolContext.messageChannel.trim()
    : '';
  const senderId = typeof toolContext.requesterSenderId === 'string'
  ...
  const isSharedSecretReadOnlyInvoke = channel === HTTP_INGRESS_CHANNEL
    && toolContext.agentId === CRM_CHANNEL
    && toolContext.senderIsOwner === true
    && !senderId
    && !accountId
  ...
  if (!isSharedSecretReadOnlyInvoke) {
    throw new Error('CRM read-only invoke requires the authenticated shared-secret gateway context');
  }
...
function trustedSelectionCacheKey(actor) {
  ...
  if (!['jorigin-crm', 'openclaw-weixin'].includes(channel) || !sessionKey) return '';
