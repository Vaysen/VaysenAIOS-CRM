...
function ownerOnlyTool({
  ...
  buildBody,
  mapResult,
  allowSharedSecretReadOnlyInvoke = false,
}) {
  return {
    name,
    label,
    ...
    factory({ config, toolContext }) {
      ...
      try {
        ...
        if (!allowSharedSecretReadOnlyInvoke) {
          return null;
        }
        try {
          baseActor = resolveTrustedReadOnlyInvokeActor(toolContext);
        } catch {
          return null;
        }
      }
...
const toolEntry = defineToolPlugin({
  id: 'jorigin-crm',
  name: 'J-Origin CRM Tools',
  description: 'Fail-closed, HMAC-authenticated tools for the J-Origin CRM boundary.',
  configSchema,
  tools: () => [
    ownerOnlyTool({
      name: 'crm_work_brief',
      allowSharedSecretReadOnlyInvoke: true,
